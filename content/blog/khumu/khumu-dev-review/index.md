...
기간이 길어지다보니 단순히 개발 놀이터보다는 진짜 출시를 목표**로 해보게 되었습니다.

그리고 대부분의 코드를 오픈소스로 관리하고 있는데 개인적으로는 학생들이 실제 프로덕션 수준의 작업에 참여해볼 수 있는 경험이
많지 않은데 우리 학교 학생들은 자신이 사용하는 오픈소스 서비스인 쿠뮤에 자유롭게 기여할 수 있는 기회의 장이 되었으면 하는
...
기존에는 댓글 서비스가 게시글 DB를 직접 조회해서 article.author와 comment.author를 비교했습니다. 하지만 이렇게 독립된 서로 다른 서비스가
같은 데이터를 조작하는 형태는 MSA에서는 매우 안 좋게 평가되는 안티 패턴입니다.
...
겪은 이슈 관련 톡을 가져와 봤습니다. 서로 포지션이 다른 경우 특히나 이슈에 대해 자세히 묘사하려 노력하고 팀원의 작업에 항상 감사를 표하는 걸 잊지 않으려
